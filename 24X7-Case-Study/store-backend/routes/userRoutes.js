...
router.post("/api/v1/admin/users", (req, res) => {
  const { firstName, lastName, email, phone, interests, address } = req.body;
  const user = new user(firstName, lastName, email, phone, interests, address);
  user.save();
  res.json({ status: "success" });
});

router.put("/api/v1/admin/users/:user_id/edit", (req, res) => {
  const { firstName, lastName, email, phone, interests, address } = req.body;
  const user = new user(firstName, lastName, email, phone, interests, address);
  user
...
router.delete("/api/v1/admin/users/:id", (req, res) => {
  user.findByIdAndDelete(req.params.id).then((u) => {
    if (u != null) {
